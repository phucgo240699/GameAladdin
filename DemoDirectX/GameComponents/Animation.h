...
public:
    ...
    Animation(const char* pathSpriteSheet, const char * pathAnimationInfo, float _timePerFrame = 0.1f, D3DCOLOR colorKey = NULL);
	...
	int id=0;// 1: Standing, 0: others
protected:
	Sprite * spriteSheet;
	vector<Tile*> listAnimationInfo;// Tile mean frame
	Tile * currentFrame;
	RECT rect;
	ifstream infile;
	int totalFrame, currentFrameIndex, lastFrameIndex;
	float timePerFrame, currentTotalTime;
	int count = 1;


    //su dung cho ke thua
    void InitWithAnimation(const char* pathSpriteSheet, const char * pathAnimationInfo, float _timePerFrame = 0.9f, D3DCOLOR colorKey = NULL);
